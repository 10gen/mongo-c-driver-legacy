...
void mongo_init( mongo *conn ) {
    ...
    conn->conn_timeout_ms = 5000;
    conn->op_timeout_ms = 5000;
}
...
int mongo_connect( mongo *conn , const char *host, int port ) {
    ...
    int conn_millis = conn->conn_timeout_ms;
    int op_millis = conn->op_timeout_ms;
    mongo_init( conn );
    mongo_set_op_timeout(conn, op_millis);
    mongo_set_connect_timeout(conn, conn_millis);
...
int mongo_find_one( mongo *conn, const char *ns, bson *query, bson *fields, bson *out ) {
...
int mongo_simple_int_command( mongo *conn, const char *db, const char *cmdstr, int arg, bson *realout ) {
    bson out = {NULL, 0};
    bson cmd;
